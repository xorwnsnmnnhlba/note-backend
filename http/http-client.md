...
### TCP와 UDP
- 전송 계층의 대표적인 프로토콜이며, 각 프로토콜의 특징과 TCP 3 way handshake에 대한 상세한 내용은 [인터넷 네트워크](/http/internet-network.md) 참고
...
- 이에 따라 URL을 URI라 통칭하기도 하지만, 엄밀히 따지면 차이가 있다
  ...
  - 위 URI 형식에서 scheme://host:port/path까지를 URL이라 볼 수 있다
    - 서로 다른 쿼리 파라미터를 가지고 요청이 들어오는 경우, 같은 URL이 포함된 서로 다른 URI라 볼 수 있다
  - 각 구성 요소에 대한 상세한 내용은 [URI와 웹 브라우저 요청 흐름](/http/uri-and-request-flow.md) 참고
