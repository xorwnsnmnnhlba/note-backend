...
- 애노테이션을 사용해서 객체를 Bean으로 등록할 수 있음
- 의존성 주입을 받으려면 Bean이 되어야하며, IoC 컨테이너 안에 들어있는 객체들끼리만 의존성 주입을 해줌
- 장점
  - 의존성 관리가 용이함
  - 스코프
  ...
  - 라이프사이클 인터페이스를 지원해줌
    - Bean이 만들어졌을 때, 필요한 시점에 추가 작업을 할 수 있음
    - 스프링 자체의 라이프사이클 콜백을 이용하여 부가적인 작업도 가능함

<br>

### 싱글턴 패턴(Singleton Pattern)
- 애플리케이션 전반에 사용되는 해당 Bean의 인스턴스가 오직 하나뿐인 것을 의미
- Bean의 기본 스코프이며, 성능적인 측면에서나 리소스 효율성 측면에서 유리함
...
- Spring에서 사용하는 Bean에 대해 익힐 수 있었다. Bean의 스코프가 싱글턴인 이유에 대해 익힐 수 있었다
