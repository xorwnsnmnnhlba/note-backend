...
### 데이터베이스 암호화
* 데이터베이스에 저장된 데이터의 암호화가 필요할 때가 있는데, 암호화와 키 관리 위치에 따라 아래와 같이 네 가지 방식으로 구분함.
* 암호화 적용 환경과 여건을 고려하여 가장 적합한 방식을 선택해야 함.
  * API 방식
    * 애플리케이션 서버 모듈에 암호화 모듈을 구성하는 방식. 애플리케이션 단에서 데이터를 암호화하여 데이터베이스에 저장함.
    * DBMS에 종속되지 않으며, 데이터베이스 과부하를 막을 수 있음.
    * 애플리케이션에서 암호화 대상 데이터와 관련있는 부분들을 모두 수정해야 할 수 있으므로, 작업량이 많아질 수 있음.
    * 암호화된 데이터가 들어감으로써, 데이터베이스의 기능에 영향을 줄 수 있음.
  * 플러그인 방식
    * DBMS에 암호화를 수행하는 모듈을 적용하는 방식. DBMS 패키지 방식이라고도 함.
    * 데이터에 세부적으로 암호화를 적용하고 관리할 수 있음.
    * 암호화/복호화를 수행할 때 DBMS에 있는 리소스를 사용하게 되므로, 이에 대한 성능을 고려해야 함.
  * 하이브리드 방식
    * API 방식과 플러그인 방식을 적절하게 섞어서 사용하는 방식. 데이터에 따라 어떠한 방식을 사용할 지 결정함.
    * 두 방식의 장점을 동시에 얻을 수 있으나, 구조적으로 복잡하고 키 관리의 복잡성도 증가함.
    * 전체 시스템을 정확하게 분석하여 상황에 맞게 적용해줘야 함.
  * TDE 방식
    * DBMS가 제공하는 암호화 기능을 이용하는 방식. DBMS가 저장소에 저장된 데이터를 통째로 암호화한다고 볼 수 있음.
    * 저장소가 물리적으로 도난당했을 때 데이터를 보호하는 수단이라 볼 수 있음.
